...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'invoices_discount_value_non_negative_check' AND conrelid = 'public.invoices'::regclass) THEN
    ALTER TABLE public.invoices ADD CONSTRAINT invoices_discount_value_non_negative_check CHECK (COALESCE(discount_value, 0) >= 0) NOT VALID;
  END IF;
...
-- Invoice Activity Logs: owner can read and append immutable activity rows
...
-- Ensure invoice client ownership and item ownership stay consistent.
