    ...
    <div className="invoice-theme-minimal bg-white p-8 text-black font-light">
      ...
      <div className="mb-10 flex items-start justify-between">
        ...
        <div className="text-right">
          <p className="text-5xl font-thin tracking-tight uppercase">فاتورة</p>
      ...
      <div className="mb-8 grid grid-cols-2 gap-8">
        <div>
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">من</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">بيانات العميل</p>
          <div className="text-sm space-y-1">
            <p>{client?.name}</p>
      ...
      <div className="mb-8 grid grid-cols-2 gap-8">
        <div>
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">تاريخ الإصدار</p>
          <p className="text-sm">{formatDateShort(inv?.issue_date)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">تاريخ الاستحقاق</p>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-black">
              <th className="py-3 text-left text-xs uppercase tracking-widest font-medium">البند</th>
              <th className="py-3 text-center text-xs uppercase tracking-widest font-medium w-20">الكمية</th>
              <th className="py-3 text-right text-xs uppercase tracking-widest font-medium w-28">السعر</th>
              <th className="py-3 text-right text-xs uppercase tracking-widest font-medium w-28">الإجمالي</th>
            </tr>
          </thead>
      ...
      <div className="flex justify-end">
        <div className="w-64 space-y-2">
          ...
          {inv?.tax_enabled && inv?.tax_rate > 0 && inv?.tax_total > 0 && (
            <div className="flex justify-between text-sm py-2 border-b border-gray-200">
              <span className="text-gray-500">ضريبة القيمة المضافة ({inv?.tax_rate}%)</span>
      ...
      {(inv?.notes || inv?.terms || inv?.payment_info) && (
        <div className="mt-8 border-t border-gray-200 pt-6">
          <div className="grid grid-cols-2 gap-8 text-sm">
            {inv?.notes && (
              <div>
                <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">ملاحظات</p>
                <p className="text-gray-600">{inv.notes}</p>
              </div>
            )}
            {(inv?.terms || inv?.payment_info) && (
              <div>
                <p className="text-xs uppercase tracking-widest text-gray-400 mb-2">بيانات الدفع</p>
